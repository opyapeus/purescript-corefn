...
import Foreign (F, Foreign, ForeignError(..), fail, isArray, readArray, readBoolean, readChar, readInt, readNull, readNumber, readString)
...
literalFromJSON t =
  object \json -> do
    ...
    case type_ of
      "IntLiteral" -> NumericLiteral <<< Left <$> (readProp "value" json >>= readInt)
      "NumberLiteral" -> NumericLiteral <<< Right <$> (readProp "value" json >>= readNumber)
      "StringLiteral" -> parseStringLiteral json
      "CharLiteral" -> CharLiteral <$> (readProp "value" json >>= readChar)
      "BooleanLiteral" -> BooleanLiteral <$> (readProp "value" json >>= readBoolean)
  ...
  parseStringLiteral json = do
    ...
    if isArray x then do
      xs <- readArray x >>= traverse readInt <#> traverse Char.fromCharCode
      case map String.fromCharArray xs of
        Nothing -> fail $ ForeignError $ "Cannot parse string literal: invalid char code"
...
moduleNameFromJSON json =
  map ModuleName $ readArray json
...
moduleFromJSON = parseJSON >=> moduleFromJSON'
  ...
  moduleFromJSON' =
    object \json -> do
      ...
      moduleImports <-
        readProp "imports" json
          >>= readArray
          >>= traverse (importFromJSON modulePath)
      moduleExports <-
        readProp "exports" json
          >>= readArray
      ...
      moduleForeign <-
        readProp "foreign" json
          >>= readArray
          >>= traverse identFromJSON
      moduleComments <-
        readProp "comments" json
          >>= readArray
  ...
  commentFromJSON json =
    ...
    blockCommentFromJSON = readProp "BlockComment" >=> map BlockComment <<< readString

    lineCommentFromJSON :: Foreign -> F Comment
    lineCommentFromJSON = readProp "LineComment" >=> map LineComment <<< readString
...
exprFromJSON modulePath =
  ...
  caseFromJSON json = do
    ann <- readProp "annotation" json >>= annFromJSON modulePath
    cs <-
      readProp "caseExpressions" json
        >>= readArray
        >>= traverse (exprFromJSON modulePath)
    cas <-
      readProp "caseAlternatives" json
        >>= readArray
